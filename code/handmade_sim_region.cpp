...
LoadEntityReference(game_state *GameState, sim_region *SimRegion, entity_reference *Ref )
{
	if (Ref->Index)
	{
		...
		if (Entry->Ptr == nullptr)
		{
			...
			Ref->Ptr = AddEntity(GameState, SimRegion, Ref->Index, GetLowEntity(GameState, Ref->Index), nullptr );
		}
		Ref->Ptr = Entry->Ptr;
	}
}
...
GetSimSpaceP( sim_region *SimRegion, low_entity *Stored )
{
	...
	// TODO(nfauvet): do we want to set this to signaling NAN
	// in debug mode to make sure nobody ever uses the position
